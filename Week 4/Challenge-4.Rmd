  ...
  filter(sentiment>0) %>%
    group_by(sender)%>%
      summarise(sender, mean_sentiment = mean(sentiment))%>%
          distinct(sender, mean_sentiment) %>% slice_max((mean_sentiment), n = 3)
**Solution:**
...
comm_data. %>%
    group_by(date)%>%
      summarise(date, all_char = sum(nchar(message)))%>%
